...
**日期**: 2025-11-10  
**作者**: TinyEngine Team  
**目标**: 设计一套可插拔的渲染优化系统，支持运行时切换和性能对比
...
### **核心理念**
采用**策略模式 + 管线化设计**，将渲染优化做成可插拔的模块，支持：

1. ✅ **运行时切换** - 无需重新编译，实时开关优化
2. ✅ **性能对比** - 同一场景下测试不同优化组合的效果
3. ✅ **零侵入** - 不破坏现有代码结构，易于集成
4. ✅ **易扩展** - 新增优化策略只需实现接口
5. ✅ **独立测试** - 每个策略可以单独开发和验证

### **优化模块**
- **Culling 剔除模块** - 3种策略（无剔除、单线程、多线程）
- **Batching 批处理模块** - 2种策略（无批处理、SRP Batch）
- **Instancing 实例化模块** - 可选开关
...
场景对象列表 (GameObject[])
         ↓
    ┌─────────────────────┐
    │  Culling 策略选择   │ ← 根据 RenderSettings
    │  ├─ NoCulling       │    选择具体策略
    │  ├─ Frustum (单线程)│
    │  └─ Frustum (多线程)│
    └─────────────────────┘
...
可见物体列表 (RenderPacket[])
         ↓
    ┌─────────────────────┐
    │  Batching 策略选择  │ ← 根据 RenderSettings
    │  ├─ NoBatching      │    选择具体策略
    │  └─ SRPBatching     │
    └─────────────────────┘
         ↓
...
namespace EngineCore
{
    ...
    struct CullingStats
    {
        int totalObjects = 0;       // 总对象数
        int visibleObjects = 0;     // 可见对象数
        int culledObjects = 0;      // 被剔除对象数
        float cullingTime = 0.0f;   // 剔除耗时（ms）
    ...
    public:
        ...
        /// <param name="allObjects">场景中所有对象</param>
        /// <param name="outContext">输出的渲染上下文（填充可见物体）</param>
...
public:
                ...
                RenderContext& outContext) override
    {
        ...
        for (auto* go : allObjects)
        {
            ...
            // 创建可见物体项
            auto* item = outContext.GetAvalileRenderPacket();
            ...
            outContext.cameraRenderPackets.push_back(item);
            mStats.visibleObjects++;
        }
        
        mStats.culledObjects = 0;  // 无剔除
...
public:
                ...
                RenderContext& outContext) override
    {
        ...
        {
            // 合并可见物体列表
            outContext.cameraRenderPackets.insert(
                outContext.cameraRenderPackets.end(),
                result.items.begin(),
                result.items.end());
...
namespace EngineCore
{
    ...
    struct RenderBatch
    {
        ...
        std::vector<Matrix4x4> worldMatrices;  // 所有实例的世界矩阵
        int instanceCount = 0;
        
        /// <summary>
        /// 生成排序键（用于材质排序）
        /// 格式：[Shader(32bit)][Material(16bit)][Mesh(16bit)]
        /// </summary>
    ...
    {
        int originalDrawCalls = 0;   // 原始DrawCall数（无批处理）
        int batchCount = 0;           // 批次数（批处理后）
        float batchingRate = 0.0f;    // 合批率（0.0-1.0）
        float batchingTime = 0.0f;    // 批处理耗时（ms）
        int materialSwitches = 0;     // 材质切换次数
        int shaderSwitches = 0;       // Shader切换次数
        std::string strategyName;     // 策略名称
    };
}
```
...
public:
    ...
    /// 构建批次
    /// </summary>
    /// <param name="RenderPackets">可见物体列表</param>
    /// <returns>渲染批次列表</returns>
...
public:
        ...
        const std::vector<RenderPacket*>& RenderPackets) override
    {
        ...
        batches.reserve(RenderPackets.size());
        
        for (auto* item : RenderPackets)
        {
            RenderBatch batch;
        ...
        // 统计
        mStats.originalDrawCalls = RenderPackets.size();
        mStats.batchCount = batches.size();
        mStats.batchingRate = 0.0f;  // 无合批
        mStats.materialSwitches = batches.size();  // 每次都切换
...
```cpp
/// <summary>
/// 策略2：SRP Batch - 排序 + 合并相同材质的物体
/// 用途：减少状态切换，支持GPU Instancing
/// </summary>
...
public:
        ...
        const std::vector<RenderPacket*>& RenderPackets) override
    {
        ...
        // ============================================
        // 步骤1：排序（Shader > Material > Mesh）
        // ============================================
        std::vector<RenderPacket*> sortedItems = RenderPackets;
        std::sort(sortedItems.begin(), sortedItems.end(), 
            [](const RenderPacket* a, const RenderPacket* b)
        {
            // 优先级1：Shader（切换Shader最昂贵）
        ...
        mStats.originalDrawCalls = RenderPackets.size();
        mStats.batchCount = batches.size();
        mStats.batchingRate = 1.0f - (float)batches.size() / RenderPackets.size();
...
public:
        ...
        const std::vector<RenderPacket*>& RenderPackets)
    {
        ...
        // 执行批处理
        auto batches = strategy->BuildBatches(RenderPackets);
...
namespace EngineCore
{
    ...
    public:
        ...
        int cullingThreadCount = 4;  // 剔除线程数（默认4）
        ...
        {
            Normal,              // 所有优化关闭（基准）
            FrustumCullingOnly,  // 只启用Frustum剔除
            MultithreadOnly,     // Frustum + 多线程
            BatchingOnly,        // Frustum + Batching
            FullOptimization     // 所有优化开启
        };
...
void RenderEngine::Render()
{
    ...
    // 获取可见物体列表
    // renderContext.cameraRenderPackets 已填充
...
void Renderer::Submit(const RenderPassInfo& info)
{
    ...
    // 步骤1：Batching（可插拔策略）
    // ============================================
    const auto& RenderPackets = info.RenderPackets;  // 从Pass传入
    auto batches = RenderBatchManager::BuildBatches(RenderPackets);
...
namespace EngineEditor
{
    ...
    struct PerformanceSnapshot
    {
        ...
        // 帧率数据
        float avgFPS;                   // 平均FPS
        float avgFrameTime;             // 平均帧时间（ms）
        
        // 渲染数据
        int avgDrawCalls;               // 平均DrawCall数
        int avgBatches;                 // 平均批次数
        float avgBatchingRate;          // 平均合批率
        
        // 剔除数据
        int avgVisibleObjects;          // 平均可见物体数
        float avgCullingRate;           // 平均剔除率
        float avgCullingTime;           // 平均剔除耗时（ms）
        
        // 采样数据
        int sampleCount = 0;            // 采样帧数
...
// 基准：30 FPS
// +Frustum：60 FPS（2倍提升）
// +Multithread：90 FPS（3倍提升）
// +Batching：120 FPS（4倍提升）
```
...
```
用户操作：
1. Assets → Create Scene → Test Scene (1000 Objects)
2. 打开 Render Options 面板
3. 切换 Performance Mode 下拉框：
   - Normal → 记录快照 → FPS: 30
   - Frustum Culling Only → 记录快照 → FPS: 60
   - + Multithread → 记录快照 → FPS: 90
   - Full Optimization → 记录快照 → FPS: 120
4. 查看 Performance Comparison 表格
   - 一目了然地看到每个优化的效果
```
...
| 方面 | 传统方式 | 可插拔模式 |
|------|---------|-----------|
| **开关切换** | 需要修改代码重新编译 | 运行时实时切换 |
| **性能对比** | 需要多次编译测试 | 一次运行多次对比 |
| **代码侵入** | 到处都是if-else判断 | 策略独立，零侵入 |
| **易扩展** | 修改复杂，容易出错 | 实现接口即可 |
| **测试** | 难以隔离测试 | 每个策略独立测试 |
| **维护** | 耦合严重，难维护 | 模块化，易维护 |
...
通过**策略模式 + 管线化**设计，我们实现了：

1. ✅ **3种Culling策略**：无剔除、单线程、多线程
2. ✅ **2种Batching策略**：无批处理、SRP Batch
3. ✅ **运行时切换**：不需要重新编译
4. ✅ **性能对比**：可视化对比不同优化的效果
5. ✅ **易于扩展**：新增策略只需实现接口

这套系统既能帮助我们**调试优化效果**，又能让我们**快速验证新策略**，是一套非常实用的架构设计。
