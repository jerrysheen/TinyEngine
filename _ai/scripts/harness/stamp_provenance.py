...
def ensure_provenance_header(path: Path, task_id: str, step_id: str | None, agent: str) -> bool:
    """Insert a provenance header for supported text files.

    Returns True when the file was changed, False otherwise. Unsupported file
    types are ignored by returning False.

    TinyEngine now keeps provenance headers as an append-only stack at the top
    of the file so later readers can see a lightweight edit trail directly in
    the file contents instead of only the latest writer.
    """
