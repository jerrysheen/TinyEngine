...
- 手动重试：`python _ai/scripts/harness/retry_step.py --task-id <task-id> --confirm-human --clear-last-error`
...
5. 当前 step 完成后，显式运行 `python _ai/scripts/harness/verify_task.py --task-id <task-id>`。
...
4. 修完后重新运行 `python _ai/scripts/harness/verify_task.py --task-id <task-id>`。
5. 如果 verifier 通过，harness 会自动推进到下一个 step；如果失败但未超上限，继续停在当前 step。
6. 如果 step 进入 `blocked`，不要继续盲试；停下来排查真实原因，由人确认问题已经修复后，才允许手动运行 `python _ai/scripts/harness/retry_step.py --task-id <task-id> --confirm-human --clear-last-error`。
7. `retry_step.py` 不是自动恢复机制，而是人工确认后的显式解锁动作；未带 `--confirm-human` 不允许执行。
8. `PreToolUse` 会在 step 已 `blocked` 或重试次数已超有效上限时直接拦截新的 Bash 命令。
